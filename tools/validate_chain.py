...
QUESTION = ("A patient has bloody diarrhea with pseudopolyps. Greatest risk?\n"
            "A. HUS\nB. Oral ulcers\nC. Colorectal cancer\nD. Pancreatic cancer")


def b_norm(model, adapter: str) -> float:
    return sum(p.detach().float().norm().item() ** 2
               for n, p in model.named_parameters()
               if "lora_B" in n and adapter in n) ** 0.5
...
def main() -> int:
    ...
    ap.add_argument("--probe-adapter", default="medical_lora",
                    help="adapter to perturb when testing APPLY and TRANSFER")
    a = ap.parse_args()
    dtype = a.dtype or ("bfloat16" if a.device == "cuda" else "float32")

    s = LatentMASSystem(model_name=a.model, device=a.device, dtype=dtype,
                        latent_steps=a.latent_steps)
    for cfg in (AgentConfig.planner(max_tokens=40), AgentConfig.medical(max_tokens=40),
                AgentConfig.critic(max_tokens=40), AgentConfig.judger(max_tokens=60)):
        s.add_agent(cfg)
    ...
    res = s.run(question=QUESTION, pipeline="true_latent", agents=chain,
    ...
    for o in res.agent_outputs:
        print(f"     {o['agent']:<15} adapter={o['adapter']:<16} mode={o.get('mode','-'):<18}"
    ...
    def expert_hidden():
        ...
        r = p.reasoner.reason(input_ids=e["input_ids"].to(dev),
    ...
    def judger_logits():
        ...
        for name in chain[:-1]:
            ...
            e = tok(p.executor.build_prompt(cfg, QUESTION, ""), return_tensors="pt",
        ...
        with torch.no_grad():
            out = s.model(input_ids=iid, attention_mask=mask, past_key_values=cache,
    ...
    print(f"     top-1 token: {tok.decode([int(l0.argmax())])!r} -> "
    ...
    print("  Note: a live pathway is not the same as a useful one. These checks\n"
