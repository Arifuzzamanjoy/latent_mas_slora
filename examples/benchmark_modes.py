...
def run_benchmark():
    ...
    for mode_name in ["true_latent", "fast", "normal"]:
        ...
        print(
            f"{emoji} {mode_name:<12} {total_t:>10.1f}s    {avg_t:>8.1f}s    {total_tok:>10}      {avg_tok:>8.0f}"
        )
    ...
    if mode_totals["normal"]["time"] > 0 and mode_totals["true_latent"]["time"] > 0:
        ...
        speedup_fast = (
            mode_totals["normal"]["time"] / mode_totals["fast"]["time"]
            if mode_totals["fast"]["time"] > 0
            else 0
        )

        token_reduction_tl = (
            (1 - mode_totals["true_latent"]["tokens"] / mode_totals["normal"]["tokens"]) * 100
            if mode_totals["normal"]["tokens"] > 0
            else 0
        )
        token_reduction_fast = (
            (1 - mode_totals["fast"]["tokens"] / mode_totals["normal"]["tokens"]) * 100
            if mode_totals["normal"]["tokens"] > 0
            else 0
        )
    ...
    if mode_totals["normal"]["time"] > 0 and mode_totals["true_latent"]["time"] > 0:
        results["summary"]["speedup"] = {
            "true_latent_vs_normal": round(
                mode_totals["normal"]["time"] / mode_totals["true_latent"]["time"], 2
            ),
            "fast_vs_normal": round(mode_totals["normal"]["time"] / mode_totals["fast"]["time"], 2)
            if mode_totals["fast"]["time"] > 0
            else 0,
        }
